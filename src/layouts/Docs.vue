        ...
        <transition-group name="menu-item" tag="div">
          <template v-if="links" v-for="(group, i1) in links">
            ...
            <template v-for="(item, i2) in group.items">
              ...
              <ul v-if="item.link === currentPath && subtitles && subtitles.length"
                  :key="`submenu-${i1}-${i2}`" class="menu-item submenu">
...
export default {
  ...
  computed: {
    ...
    editLink() {
      ...
      if ((path.match(new RegExp('/', 'g')) || []).length == 1) path += '/index';
      return `https://github.com/FrontlineEducation/dev-portal/blob/master${path}.md`;
    },
    items() {
